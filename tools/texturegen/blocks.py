...
@block("fusion_electromagnet")
def fusion_electromagnet():
    t = Tex(seed=41)
    brushed(t, DARK, base=2.6, amp=0.5, seed=41)
    # coil windings wrapped around the beam tube (vertical turns)
    for x in range(2, 14):
        k = (x - 2) % 3
        c = (COPPER[5], COPPER[3], COPPER[1])[k]
        for y in range(2, 14):
            t.set(x, y, c)
        t.set(x, 2, COPPER[6] if k == 0 else COPPER[4])
        t.set(x, 13, COPPER[0])
    # cryogenic bus bars top and bottom
    for y in (0, 1, 14, 15):
        t.hline(0, 15, y, CRYO[2] if y in (1, 14) else CRYO[1])
    for x in range(1, 15, 4):
        t.set(x, 1, CYAN[6])
        t.set(x, 14, CYAN[6])
    glow(t, 8, 1, 4.5, CYAN[6], 0.22)
    glow(t, 8, 14, 4.5, CYAN[6], 0.22)
    outline(t, 0, 0, 15, 15, DARK[0])
    return t


@block("fusion_magnet_input")
def fusion_magnet_input():
    t = fusion_electromagnet()
    # power coupling in the middle
    t.rect(4, 4, 11, 11, DARK[2])
    bevel(t, 4, 4, 11, 11, CRYO)
    outline(t, 3, 3, 12, 12, DARK[0])
    t.disc(8, 8, 3.0, CRYO[2])
    t.ring(8, 8, 3.0, 2.2, CYAN[4])
    t.disc(8, 8, 1.6, CYAN[6])
    for a in (0, 90, 180, 270):
        r = math.radians(a)
        t.line(8 + math.cos(r) * 2.0, 8 + math.sin(r) * 2.0,
               8 + math.cos(r) * 3.6, 8 + math.sin(r) * 3.6, CYAN[5])
    glow(t, 8, 8, 6.0, CYAN[6], 0.3)
    return t
...
def fusion_accelerator_segment():
    ...
    t.hline(0, 15, 5, CRYO[6])
    t.hline(0, 15, 10, CRYO[1])
    t.hline(0, 15, 11, CRYO[0])
    # beam channel
    t.rect(0, 7, 15, 8, DARK[0])
    t.hline(0, 15, 7, CYAN[5])
    t.hline(0, 15, 8, CYAN[3])
    # confinement rings
    for x in (2, 8, 13):
        t.vline(x, 3, 12, CRYO[5])
        t.vline(x + 1, 3, 12, CRYO[1])
        t.set(x, 7, CYAN[6])
        t.set(x + 1, 7, CYAN[4])
    glow(t, 8, 7.5, 7.0, CYAN[6], 0.20)
    outline(t, 0, 0, 15, 15, DARK[0])
    return t
...
def fusion_accelerator_corner():
    ...
    t.hline(0, 11, 4, CRYO[6])
    t.vline(11, 4, 15, CRYO[1])
    t.hline(0, 4, 11, CRYO[1])
    t.vline(4, 11, 15, CRYO[6])
    # beam channel bends with the shell
    t.rect(0, 7, 8, 8, DARK[0])
    t.rect(7, 7, 8, 15, DARK[0])
    t.hline(0, 8, 7, CYAN[5])
    t.vline(7, 7, 15, CYAN[5])
    t.hline(0, 7, 8, CYAN[3])
    t.vline(8, 8, 15, CYAN[3])
    # confinement rings on each leg
    for x in (1, 4):
        t.vline(x, 4, 6, CRYO[5])
        t.vline(x, 9, 11, CRYO[5])
    for y in (12, 14):
        t.hline(5, 7, y, CRYO[5])
        t.hline(9, 11, y, CRYO[5])
    glow(t, 7.5, 7.5, 7.0, CYAN[6], 0.18)
    outline(t, 0, 0, 15, 15, DARK[0])
    return t
...
def fusion_fuel_injector():
    ...
    outline(t, 3, 1, 12, 6, CRYO[0])
    for i in range(4):
        t.rect(5 + i, 6 + i, 10 - i, 6 + i, CRYO[4 - i // 2])
        t.set(5 + i, 6 + i, CRYO[6])
        t.set(10 - i, 6 + i, CRYO[1])
    # injected fuel plume
    t.rect(7, 10, 8, 11, TRITIUM[5])
    t.rect(6, 12, 9, 13, TRITIUM[4])
    t.rect(5, 14, 10, 14, TRITIUM[3])
    t.set(7, 10, WHITE)
    glow(t, 8, 12, 6.0, TRITIUM[6], 0.32)
    return t


@block("fusion_plasma_turbine")
def fusion_plasma_turbine():
    t = Tex(seed=46)
    casing(t, DARK, seed=46, seam=False)
...
@block("fusion_controller_off")
def fusion_controller_off():
    return _fusion_controller("off")


@block("fusion_controller_on")
def fusion_controller_on():
    return _fusion_controller("on")


@block("fusion_controller_ignited")
def fusion_controller_ignited():
    return _fusion_controller("ignited")
...
def _fusion_controller(state):
    ...
    if state == "on":
        # plasma containment ring readout
        t.ring(8, 6, 2.8, 2.0, CYAN[6])
        t.set(8, 6, CYAN[6])
    elif state == "ignited":
        t.ring(8, 6, 3.0, 2.2, PLASMA[6])
        t.disc(8, 6, 1.6, WHITE)
        glow(t, 8, 6, 6.5, PLASMA[6], 0.5)
    # status bar
    for i, x in enumerate((4, 7, 10)):
        c = {"off": CRYO[2], "on": CYAN[5], "ignited": PLASMA[5]}[state]
        t.rect(x, 11, x + 1, 12, c)
        t.set(x, 11, {"off": CRYO[3], "on": CYAN[6], "ignited": PLASMA[6]}[state])
        t.set(x + 1, 12, CRYO[0])
    t.hline(3, 12, 10, CRYO[1])
    return t
...
def fusion_turbine_fluid_port_input():
    t = port_face(Tex(seed=50), CRYO, IN_ACCENT, seed=50, out=False)
    vents(t, 1, 1, 14, 2, CRYO, step=1)
    vents(t, 1, 13, 14, 14, CRYO, step=1)
    return t


@block("fusion_turbine_fluid_port_output")
def fusion_turbine_fluid_port_output():
    t = port_face(Tex(seed=51), CRYO, OUT_ACCENT, seed=51, out=True)
    vents(t, 1, 1, 14, 2, CRYO, step=1)
    vents(t, 1, 13, 14, 14, CRYO, step=1)
    return t
